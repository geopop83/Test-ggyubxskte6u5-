    ...
    if (speciale) {
      colLeft.innerHTML = `
        <div class="side-article" onclick="apriArticolo('cercasi-collaboratori')">
          ...
          <span class="card-link" style="display:block;margin-top:12px;color:var(--ink);opacity:1">Leggi →</span>
        </div>`;
    }
  }
...
// APRI ARTICOLO
// ══════════════════════════════════════════════════════
// Traccia la pagina di provenienza per il tasto indietro degli articoli
let _provenienza = 'home';

function tornaIndietroArticolo() {
  if (_provenienza === 'archivio') {
    renderArchivio();
  } else {
    document.getElementById('page-articolo').style.display = 'none';
...
function apriArticolo(id) {
  // Salva da dove si apre — solo page-archivio del sito principale conta come archivio
  _provenienza = _paginaCorrente === 'page-archivio' ? 'archivio' : 'home';
  ...
  document.getElementById('articolo-contenuto').innerHTML = `
    <div class="articolo-back" onclick="tornaIndietroArticolo()">← Indietro</div>
    ...
    <div class="articolo-firma">
      ...
      ${autore.cliccabile === false
        ? `<span class="firma-nome" style="cursor:default">${autore.nome}</span>`
        ...
        ${ordinati.map(c => `
          <div class="team-card" onclick="apriAutore('${c.id}')">
            ...
            <div class="team-info">
              ...
              <div class="team-bio-breve">${c.bio ? c.bio.substring(0,100) + '...' : ''}</div>
            </div>
          </div>`).join('')}
      </div>
    </div>`;
    ...
    <div class="archivio-wrap">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:40px;max-width:800px;margin-top:8px">
        ...
        <div>
          ...
          <p style="font-family:'Playfair Display',serif;font-size:1rem;line-height:1.8;color:var(--ink);opacity:0.75;margin-bottom:16px">Scrivi, fotografi, disegni, viaggi? Geo-POPlitica è un progetto collettivo, parlaci di te, mostraci qualcosa di quello che fai tramite un link al tuo sito, blog o account social pubblico (non usare allegati) e facci sapere cosa ti piacerebbe realizzare. Collaboriamo per passione e non per compenso, ma ci teniamo a farlo bene. Siamo alla ricerca di persone curiose da ogni angolo del mondo.</p>
          <p style="font-family:'Playfair Display',serif;font-size:1rem;line-height:1.8;color:var(--ink);opacity:0.75;margin-bottom:16px">Ci prendiamo il tempo di leggere tutto quello che riceviamo. Se troviamo il posto giusto per un tuo progetto inedito, ti scriviamo.</p>
